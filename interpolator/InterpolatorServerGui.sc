InterpolatorServerGui : AbstractInterpolatorGui {
		...
		actions = IdentityDictionary[
			\weights -> {|model, what, interpoints, weights|
				guiItems.do{ |guiItem, i|
					guiItem[3].string_(
						weights[i].trunc(0.0001).asString;
					);
				}
			},
...
InterpolatorServer2DGui : AbstractInterpolatorGui {
		...
		uv.drawFunc =  {
			...
			model.points.do({ |point, i|
				...
				Pen.fillOval(Rect.aboutPoint(
					...
					model.weights[i] * bounds.extent.x * 0.2 + 5,
					model.weights[i] * bounds.extent.y * 0.2 + 5)
				);
				Pen.stroke;
			});
...
// 			[x,y,mondifiers,buttonNumber,clickCount].postln;
			scaled = this.scale(this.getPoint(model.cursor));
			model.points.do{|point,i|
				//if mouse is within 6 pixels of a point, grab it.
				point = this.scale(this.getPoint(point));
		...
		uv.mouseMoveAction = {
			...
			grabbed.if {
				(grabbedPoint == -1).if {
					...
					model.dragPoint(
						grabbedPoint,
						model.points[grabbedPoint].putEach(
							[this.x, this.y],
							this.unscale((Point(x,y) - diff)).asArray
						)
					);
				};
			};
